...
to_be_normalized <- as.data.frame(toyota.missing_100[, -c(1)])
predicted_coloumn <- as.data.frame(toyota.missing_100[, -c(2,3,4,5,6,7,8)])
print(to_be_normalized)
print(predicted_coloumn)





normalized_data<-normalize(to_be_normalized, method = "scale", range = c(0, 1))#----------------using normalization function 

print(normalized_data)


normalized_data_1<- data.frame(cbind(predicted_coloumn, normalized_data))


colnames(normalized_data_1) <- c("FuelType", "Price", "Age","KM","FuelType_orignal","CC","Weight")
print(normalized_data_1)
toyota.missing_100<-normalized_data_1
summary(df)
...
for (row in 1:nrow(toyota.missing_100)) {
  ...
  if(is.na(fuel)) {
    ...
    for (ja in 1:nrow(toyota.missing_100)){ #---------------------------getting feature 1 which is price
     ...
     # if(!is.na(km) && !is.na(price) && !is.na(cc) && !is.na(weight_cars)){
      ...
      if(!is.na(price)) {
        ...
        distance_price[ja]<-c(euclideanDist(price_compare_price, price_inside_price))
    ...
    # print(ordered_distance)
     ...
     #onlydistance<-sum_distance[, -c(1,3)]
     #print(onlydistance)
     #inv_sum_distance<-inv(sum_distance$Distance_price )
     print(sum_distance)
    ...
    ordered_distance1<-ordered_distance[ ,-c(1,2,3,4)]
    data.matrix(ordered_distance1)
    print(ordered_distance)
   ...
   # ordered_distance<---sum of all distance
    ...
    for (ba in 1:nrow(ordered_distance_k)){#---------------------creating loop for calculating classes total weigths
      
      if((ordered_distance_k[ba, "fuel_predict"] == 1)){
        
        sum_class1<- sum_class1 + (ordered_distance_k[ba, "classes_weights"])
        
        
      }
      
      
      if((ordered_distance_k[ba, "fuel_predict"] == 2)){
        
        sum_class2<- sum_class2 + (ordered_distance_k[ba, "classes_weights"])
        
      }
      
      if((ordered_distance_k[ba, "fuel_predict"] == 3)){
        
        sum_class3<- sum_class3 + (ordered_distance_k[ba, "classes_weights"])
    ...
    max_value <- max(sum_class1, sum_class2,sum_class3)#---------------------selecting max weights value
    print(max_value)
    ...
    #rounded_max_value<-round(max_value)
    ...
    #for(q in 1:nrow(toyota.missing_100) ){
     ...
     #toyota.missing_100$FuelType[row][is.na(toyota.missing_100$FuelType[row])] <- rounded_max_value
    
    toyota.missing_100[row, "FuelType"] <- assignmet_Value#----------------------------------updating value of age in database
    #toyota.missing_100$FuelType[row]
    #print(toyota.missing_100$Age[row])
    # print(row)
    # print(toyota.missing_100)
    print(paste(rounded_max_value,"rounded max value"))
    #break()
    #}
...
accuracy(d)
print(paste("Accuracy is", accuracy(d)))

accu_d <- accuracy(d)
write.csv(d,'W:\\cases\\Question 2\\Feature Scaling 2 devide by standard daviation\\MyData_catagorical.csv', row.names = TRUE)
write.table(accu_d, 'W:\\cases\\Question 2\\Feature Scaling 2 devide by standard daviation\\MyData_catagorical.csv', col.names=FALSE, sep=",", append=TRUE)
