...
toyota.age_missing<-as.data.frame(lapply(df, function(cc) cc[ sample(c(TRUE, NA), prob = c(0.80, 0.20), size = length(cc), replace = TRUE) ]))
...
to_be_normalized <- as.data.frame(toyota.missing_100[, -c(1,3)])
predicted_coloumn <- as.data.frame(toyota.missing_100[, -c(2,4,5,6,7,8)])
...
#colnames(normalized_data_1) <- c("Age", "Price", "Age_correct","KM","FuelType","CC","Weight")
...
get_mse<- function(test_data){
  ...
  for(i in c(1:nrow(test_data))){    #--------------------------------------creating accuracy function
    ...
    mse = mse + (test_data[i,1] - test_data[i,2])**2
    mse = (1/i)*mse
    #print(mse)
  }
...
msse <- get_mse(xn)
write.csv(xn,'W:\\cases\\Question 2\\Feature scaling 1 center and scale\\Feature_scaling1_continuous.csv', row.names = TRUE)
write.table(msse, 'W:\\cases\\Question 2\\Feature scaling 1 center and scale\\Feature_scaling1_continuous.csv', col.names=FALSE, sep=",", append=TRUE)
